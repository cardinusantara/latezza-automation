...
async function runProactiveFollowUps(log = console, ignoreThreshold = false) {
  ...
  try {
    const delayHours = parseInt(await db.getSetting('followup_hours') || '24', 10);
    ...
    for (const lead of leads) {
      ...
      let userInstruction = await db.getSetting('followup_instruction');
...
${strictOutputRule}`;
      ...
      const formattedPrompt = followUpPrompt
        ...
        .replace(/\{reason\}/g, lead.follow_up_reason || 'Tertarik produk')
